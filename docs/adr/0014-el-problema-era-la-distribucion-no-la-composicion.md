...
- **`awi-core` sigue público, deliberadamente.** La revisión propuso pasarlo a privado y se descartó por una restricción de plataforma: en el plan gratuito de GitHub, un repo privado no ofrece colaboradores de sólo lectura —los permisos granulares están detrás del paywall—, así que **público sin colaboradores *es* el control de acceso** que se busca: lectura para quien lo necesite, push para nadie. Queda pendiente que el estado real coincida con la intención: hoy `ChristianSRgit` y `nazarenacolqui-ux` figuran como colaboradores con `push`.
- Con el repo público, la purga del historial pasa de higiene a requisito, y por una razón distinta de la que motivó el hallazgo. La password expuesta era de una cuenta de test y no importa. Lo que sí importa son los logs de delegate: son transcripciones de auditoría sobre código de clientes —el de `newhaze-learn` enumera vulnerabilidades concretas con severidad— y hay 139 objetos de `.claude/tmp` en el historial. Desversionarlos saca el material del árbol pero no del historial de un repo público. Sigue en fase 2, ahora como el ítem de más peso de esa fase.
