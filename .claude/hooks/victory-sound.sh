#!/bin/bash
# Read stdin to get hook JSON (advisory hook, passed via stdin)
INPUT=$(cat)
REASON=$(echo "$INPUT" | grep -iPo '"reason"\s*:\s*"\K[^"]+')

# If reason is not "exit", do not play the victory sound
# This prevents sound on terminal exit (Ctrl+C/D) or error stops
if [ "$REASON" != "exit" ]; then
    exit 0
fi
...
WAV_FILE="$SCRIPT_DIR/victory-sound.wav"
