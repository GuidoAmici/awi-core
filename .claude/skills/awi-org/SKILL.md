...
Adds an organization workspace under `_data/organizations/<name>/` — either scaffolded fresh or imported from an existing GitHub repo.
...
- **If ARGUMENTS is empty** — ask:
  ```
  What is the organization name? (e.g. acme-corp, my-client, partner-name)
  ```
...
## Mode A — New organization (choice: 1)

### A1 — Scaffold

```bash
python3 .claude/skills/awi-org/scripts/init_org.py <name>
```
...
git commit -m "cos: add organization submodule - <name>"
```
...
```bash
python3 .claude/skills/awi-org/scripts/import_client.py <name>
```
...
```bash
python3 .claude/skills/shared/scripts/log_command.py awi-org <outcome>
```
