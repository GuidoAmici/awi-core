...
from paths import AWI_ROOT, ORGANIZATIONS_DIR, ORGANIZATIONS_RELDIR
...
def get_registered_clients() -> list[str]:
    ...
    for line in result.stdout.splitlines():
        ...
        if len(parts) >= 2:
            path = parts[1]
            if path.startswith(ORGANIZATIONS_RELDIR + "/") and path.count("/") == 2:
                clients.append(path.removeprefix(ORGANIZATIONS_RELDIR + "/"))
    return clients


def is_active(name: str) -> bool:
    """True if submodule working directory has content (is initialized)."""
    client_path = ORGANIZATIONS_DIR / name
...
def enable(name: str) -> int:
    ...
    result = subprocess.run(
        ["git", "submodule", "update", "--init", "--recursive", f"{ORGANIZATIONS_RELDIR}/{name}"],
        cwd=AWI_ROOT,
    )
...
def disable(name: str) -> int:
    ...
    result = subprocess.run(
        ["git", "submodule", "deinit", "-f", f"{ORGANIZATIONS_RELDIR}/{name}"],
        cwd=AWI_ROOT,
    )
    if result.returncode == 0:
...
def status() -> int:
    ...
    for name in registered:
        ...
        print(f"  {state}  _data/organizations/{name}")
    return 0
