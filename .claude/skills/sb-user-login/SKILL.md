---
name: sb-user-login
description: Load a person's profile for this session. Usage: /sb-user-login <username>. Greets by name, loads preferences and long-term patterns, primes session context.
allowed-tools: Read, Bash, Glob
model: sonnet
subagent_type: general-purpose
---

# /sb-user-login - Load User Profile

Usage: `/sb-user-login <username>`

## User file format

User files live in `_documentation/context/users/<username>.md`. The **filename is the username**. The file must contain a `person` field linking to a file in `_documentation/schedule/people/`:
...
- `person` — must match a filename (without `.md`) in `_documentation/schedule/people/`
...
- **If ARGUMENTS is empty or blank** — list available users and STOP:
  ```bash
  ls /home/unixadmin/second-brain/_documentation/context/users/
  ```
...
4. If `person` is missing or blank — stop and warn:
   ```
   User '<username>' has no linked person. Add a `person: <Name>` field to _documentation/context/users/<username>.md.
   ```

5. Read the linked person's file at `_documentation/schedule/people/<person>.md` — name, roles, preferences, long-term patterns.

6. Read the most recent user-profile-inference entry for this person (if any):
   ```bash
   ls /home/unixadmin/second-brain/_documentation/schedule/user-profile-inference/ | sort -r | head -1
   ```
