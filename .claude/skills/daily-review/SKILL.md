...
1. Read `info/organization/daily/YYYY-MM-DD.md` for today's plan
2. Run `git log --since="8am" --grep="cos:"` for activity
3. Find relevant tasks efficiently (don't read all tasks):
   - Search for tasks due today: `grep -l "due: YYYY-MM-DD" info/organization/tasks/*.md`
   - Find task files modified today: `git diff --name-only HEAD@{8am}..HEAD -- info/organization/tasks/`
