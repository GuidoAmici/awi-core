---
group: SETUP
description: Add or import client submodule
order: 2
---

# awi-client

Add a client workspace under `_data/organizations/<name>/` — create from scratch or import from GitHub.
...
```mermaid
graph TD
    A(["/awi-client name"]) --> B{name in args?}
    B -->|no| C[/Ask for client name/]
    B -->|yes| D{Mode?}
    C --> D
    D -->|1 new| E[[python3 init_client.py name]]
